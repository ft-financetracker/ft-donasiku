(()=>{
  ...
  const CACHE_KEY='kia_public_bootstrap_v056';
  const LEGACY_CACHE_KEYS=['kia_public_bootstrap_v055','kia_public_bootstrap_v054','kia_public_bootstrap_v052','kia_public_bootstrap_v051'];
  ...
  function normalizeHeroAsset(hero,index){
    ...
    if(legacy)item.image_url=`./assets/images/hero/hero-${legacy[1]}.png`;
    else if(!image)item.image_url=DEFAULT_HEROES[Math.max(0,Math.min(2,slot-1))].image_url;
    item.sort_order=slot;return item;
  }
  function normalizedHeroes(remote){
    const active=Array.isArray(remote)?remote.filter(Boolean).slice(0,3).map(normalizeHeroAsset):[];
    const used=new Set(active.map(x=>Number(x.sort_order)||0).filter(Boolean));const result=[...active];
    DEFAULT_HEROES.forEach((fallback,index)=>{if(result.length>=3)return;const slot=index+1;if(!used.has(slot))result.push({...fallback,sort_order:slot})});
    while(result.length<3)result.push(DEFAULT_HEROES[result.length]);return result.slice(0,3);
  }
  function renderHero(){
    const h=heroes[heroIndex];if(!h)return;
    $('[data-hero-title]').textContent=h.title||'';$('[data-hero-subtitle]').textContent=h.subtitle||'';
    $('[data-hero-image]').style.backgroundImage=`url("${h.image_url||DEFAULT_HEROES[heroIndex]?.image_url||DEFAULT_HEROES[0].image_url}")`;
    const cta=$('[data-hero-cta]');cta.textContent=h.cta_label||'Mulai Berdonasi';cta.href=h.cta_url||'#program';
    $('[data-hero-dots]').innerHTML=heroes.map((_,i)=>`<button class="hero-dot ${i===heroIndex?'is-active':''}" data-dot="${i}" aria-label="Hero ${i+1}"></button>`).join('');
    $$('[data-dot]').forEach(button=>button.onclick=()=>{heroIndex=Number(button.dataset.dot);renderHero();restartHeroTimer()});
  }
  ...
  function renderLiveDonations(items){
    ...
    root.innerHTML=rows.map(x=>{
      ...
      return `<article class="card live-donation-card-v056">
        <div class="live-donation-head-v056"><strong>${idr(x.amount)}</strong><span class="donor-badge donor-badge--${badgeClass(x.donor_badge_code)}">${esc(x.donor_badge||'Umum')}</span></div>
        <div class="live-donation-program-v056" title="${esc(x.program_name||'Program KIA')}">${esc(x.program_name||'Program KIA')}</div>
        <div class="live-donation-person-v056"><strong>${esc(donor)}</strong>${message?`<span>·</span><em>“${esc(message)}”</em>`:'<span class="muted">· Tanpa pesan</span>'}</div>
      </article>`;
    }).join('')
  }
  ...
  function applyData(data){
    ...
    renderPrograms(data.programs||[]);renderLiveDonations(data.live_donations||[]);renderFaq(data.faqs||[]);
    if($('[data-stat-paid]'))$('[data-stat-paid]').textContent=idr(data.stats?.total_paid_amount);
    if($('[data-stat-programs]'))$('[data-stat-programs]').textContent=String(data.stats?.active_programs||0);
    if($('[data-stat-disbursed]'))$('[data-stat-disbursed]').textContent=idr(data.stats?.total_withdrawn_net);
  }
  ...
  async function prefetchCatalog(seed){
    const key='kia_catalog_v056_1_ALL_';
    try{
      if(seed?.programs?.length){
        const total=Number(seed.stats?.active_programs)||seed.programs.length;
        localStorage.setItem(key,JSON.stringify({t:Date.now(),d:{page:1,limit:12,total,total_pages:Math.max(1,Math.ceil(total/12)),items:seed.programs},partial:true}));
      }
