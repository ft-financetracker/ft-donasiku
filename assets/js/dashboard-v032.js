(() => {
  ...
  function renderRoute(){
    ...
    if(['account','settings','admin','cms'].includes(route) && Date.now()-state.lastSyncAt>15000){ loadBootstrap({quiet:true}).catch(()=>{}); }
  }
  ...
  async function syncAccount(showToast=true){
    if(showToast) KiaUI.showLoading({title:'Menyinkronkan akun',message:'Mengambil role dan data terbaru…'});
    try{
      const ok=await loadBootstrap({quiet:!showToast});
      if(!ok) throw new Error('Sinkronisasi belum berhasil.');
      if(showToast){KiaUI.hideLoading(); KiaUI.toast('Akses akun berhasil diperbarui.',{type:'success'});}
    }catch(err){
      if(showToast){KiaUI.hideLoading(); KiaUI.toast(err.message||'Sinkronisasi gagal.',{type:'warning'});}
    }
  }
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    ...
    if(!ok){setTimeout(()=>loadBootstrap({quiet:true}).catch(()=>{}),1800);}
  });
})();
