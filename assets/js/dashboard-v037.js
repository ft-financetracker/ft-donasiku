(() => {
  ...
  function editProgram(id){
    ...
    renderProgramCoverPreview(p.cover_image_url||'');
  ...
  function renderProgramCoverPreview(url){
    const el=$('[data-program-cover-preview]');
    if(!el) return;
    const v=String(url||'').trim();
    el.classList.toggle('has-image',!!v);
    el.style.backgroundImage=v?`url("${v.replaceAll('"','%22')}")`:'';
    el.innerHTML=v?'':'<span class="muted mini">Belum ada cover dipilih.</span>';
  }

  async function uploadProgramCover(){
    const input=$('[data-program-cover-file]');
    const file=input?.files?.[0];
    if(!file) return KiaUI.toast('Pilih gambar cover terlebih dahulu.',{type:'warning'});
    if(!['image/jpeg','image/png','image/webp'].includes(file.type)) return KiaUI.toast('Format harus JPG, PNG, atau WEBP.',{type:'error'});
    const btn=$('[data-upload-program-cover]');
    const old=btn?.textContent;
    if(btn){btn.disabled=true;btn.textContent='Mengimpor…';}
    try{
      const dataUrl=await compressImage(file);
      const r=await KiaAuth.request('/api/media/program-cover',{method:'POST',body:JSON.stringify({file_name:file.name.replace(/\.[^.]+$/,'.jpg'),mime_type:'image/jpeg',base64:dataUrl}),timeout:24000});
      const url=r.data.media.public_url;
      const f=$('#programForm');
      if(f?.elements.cover_image_url) f.elements.cover_image_url.value=url;
      renderProgramCoverPreview(url);
      input.value='';
      KiaUI.toast('Cover berhasil diimpor.',{type:'success'});
    }catch(err){KiaUI.toast(err.message||'Impor cover gagal.',{type:'error',duration:5200});}
    finally{if(btn){btn.disabled=false;btn.textContent=old||'Impor Gambar';}}
  }
  ...
  async function uploadHeroMedia(){
    ...
    if(!['image/jpeg','image/png','image/webp'].includes(file.type)) return KiaUI.toast('Format harus JPG, PNG, atau WEBP.',{type:'error'});
    const btn=$('[data-upload-hero-media]'); const old=btn?.textContent; if(btn){btn.disabled=true;btn.textContent='Mengimpor…';}
    try{
      const dataUrl=await compressImage(file);
      const r=await KiaAuth.request('/api/admin/media/upload',{method:'POST',body:JSON.stringify({file_name:file.name.replace(/\.[^.]+$/,'.jpg'),mime_type:'image/jpeg',base64:dataUrl}),timeout:24000});
      state.media=[r.data.media,...state.media.filter(x=>x.media_id!==r.data.media.media_id)];
      renderMediaLibrary();
      input.value='';
      selectMedia(r.data.media.media_id);
      KiaUI.toast('Gambar berhasil diimpor dan langsung dipilih.',{type:'success'});
    }catch(err){KiaUI.toast(err.message||'Upload gambar gagal.',{type:'error',duration:6000});}
    finally{if(btn){btn.disabled=false;btn.textContent=old||'Impor Gambar';}}
  }
  ...
  function showSettingsView(name='home'){
    $$('[data-settings-view]').forEach(el=>el.hidden=el.dataset.settingsView!==name);
  ...
  function renderSettings(){
    ...
    if(!data) return;
    $('[data-settings-current-role]')&&($('[data-settings-current-role]').textContent=data.current_user?.platform_role||state.user?.platform_role||'USER');
    $('[data-settings-version]')&&($('[data-settings-version]').textContent=data.platform?.version||'0.3.6');
    const note=$('[data-role-management-note]');
    if(note){
      note.innerHTML=data.can_manage_roles
        ?'<strong>SUPER_ADMIN aktif.</strong> Gunakan pencarian dan filter agar daftar pengguna tetap ringkas.'
        :'<strong>Role Management terkunci.</strong> Hanya SUPER_ADMIN yang dapat mengubah role platform.';
    }
  }
  ...
  async function loadSettings(force=false){
    ...
    try{
      const r=await KiaAuth.request('/api/admin/settings',{timeout:18000,cache:'no-store'});
      state.settings=r.data;
      renderSettings();
    }catch(err){
      KiaUI.toast(err.message||'Pengaturan gagal dimuat.',{type:'error'});
    }
  }
  ...
  function filteredUsers(){
    ...
    return state.users.filter(u=>{
      const roleOk=state.userRoleFilter==='ALL'||u.platform_role===state.userRoleFilter;
      const qOk=!q||String(u.full_name||'').toLowerCase().includes(q)||String(u.email||'').toLowerCase().includes(q);
      return roleOk&&qOk;
    });
  }
  ...
  function renderUsers(){
    ...
    if(!host) return;
    const canManage=!!state.settings?.can_manage_roles;
    ...
    $('[data-user-result-meta]')&&($('[data-user-result-meta]').textContent=`${rows.length} pengguna ditemukan`);
    const pager=$('[data-user-pagination]');
    if(pager) pager.hidden=rows.length<=pageSize;
    $('[data-user-page-label]')&&($('[data-user-page-label]').textContent=`${state.userPage} / ${totalPages}`);
    const prev=$('[data-user-page-prev]'); const next=$('[data-user-page-next]');
    if(prev) prev.disabled=state.userPage<=1;
    if(next) next.disabled=state.userPage>=totalPages;
    if(!pageRows.length){host.innerHTML='<div class="empty-state">Tidak ada pengguna yang cocok dengan pencarian/filter.</div>';return;}
    host.innerHTML=pageRows.map(u=>`
      <article class="user-role-item">
        <div class="user-role-main"><strong>${esc(u.full_name||u.email)}</strong><span class="muted mini">${esc(u.email)} · ${esc(u.platform_role||'USER')}</span></div>
        <div class="user-role-control">
          <select data-role-user="${esc(u.user_id)}" ${!canManage||u.user_id===state.user?.user_id?'disabled':''}>
            ${['USER','PLATFORM_ADMIN','SUPER_ADMIN'].map(role=>`<option value="${role}" ${u.platform_role===role?'selected':''}>${role}</option>`).join('')}
          </select>
          <button class="btn btn-soft" type="button" data-save-role="${esc(u.user_id)}" ${!canManage||u.user_id===state.user?.user_id?'disabled':''}>Simpan</button>
        </div>
      </article>`).join('');
  }
  ...
  async function loadUsers(force=false){
    ...
    const host=$('[data-user-role-list]');
    if(host) host.innerHTML='<div class="empty-state">Memuat pengguna…</div>';
    try{
      const r=await KiaAuth.request('/api/admin/users',{timeout:22000,cache:'no-store'});
      state.users=Array.isArray(r.data?.users)?r.data.users:[];
      state.userPage=1;
      renderUsers();
    }catch(err){
      if(host) host.innerHTML='<div class="empty-state">Pengguna belum dapat dimuat. Tekan Refresh untuk mencoba lagi.</div>';
      KiaUI.toast(err.message||'Daftar pengguna gagal dimuat.',{type:'warning'});
    }
  }
  ...
  function bind(){
    ...
    $('[data-settings-groups]')?.addEventListener('click',e=>{const head=e.target.closest('[data-settings-toggle]');if(!head)return;head.closest('.settings-group')?.classList.toggle('is-open');});
    $$('[data-settings-open]').forEach(btn=>btn.addEventListener('click',()=>showSettingsView(btn.dataset.settingsOpen)));
    $$('[data-settings-back]').forEach(btn=>btn.addEventListener('click',()=>showSettingsView('home')));
    $('[data-users-refresh]')?.addEventListener('click',()=>loadUsers(true));
    $('[data-user-search]')?.addEventListener('input',e=>{state.userQuery=e.target.value||'';state.userPage=1;renderUsers();});
    $('[data-user-role-filter]')?.addEventListener('change',e=>{state.userRoleFilter=e.target.value||'ALL';state.userPage=1;renderUsers();});
    $('[data-user-page-prev]')?.addEventListener('click',()=>{if(state.userPage>1){state.userPage--;renderUsers();}});
    $('[data-user-page-next]')?.addEventListener('click',()=>{const total=Math.max(1,Math.ceil(filteredUsers().length/state.userPageSize));if(state.userPage<total){state.userPage++;renderUsers();}});
    $('[data-upload-program-cover]')?.addEventListener('click',uploadProgramCover);
