(()=>{
  ...
  const key=p=>`kia_program_donations_v057_${programId}_${p}`;
  function readCache(p){try{for(const k of [key(p),`kia_program_donations_v056_${programId}_${p}`,`kia_program_donations_v055_${programId}_${p}`]){const x=JSON.parse(sessionStorage.getItem(k)||'null');if(x&&Date.now()-x.saved_at<CACHE_TTL)return x.data}return null}catch{return null}}
  ...
  function avatar(x){const donor=x.donor_label||'Hamba Allah',p=x.donor_profile||null,url=String(p?.avatar_url||'').trim();return `<div class="donor-avatar-frame-v057 frame-${esc(frameCode(p))}" aria-hidden="true">${url?`<img src="${esc(url)}" alt="">`:`<span>${esc(initials(donor))}</span>`}</div>`}
  function row(x,index){
    ...
    return `<article class="donation-row-v057 ${social?'is-social':'is-static'}" ${attrs}>
      <div class="donation-cell-no-v057">#${index}</div>
      <div class="donation-cell-profile-v057">${avatar(x)}</div>
      ...
      <div class="donation-cell-program-v057">${esc(x.program_name||'Program KIA')}</div>
      <div class="donation-cell-badge-v057"><span class="donor-badge donor-badge--${badgeClass(x.donor_badge_code)}">${esc(x.donor_badge||'Tamu')}</span></div>
      <div class="donation-cell-amount-v057"><strong>${idr(x.amount)}</strong><time>${esc(dateTime(x.paid_at))}</time></div>
    </article>`
  }
  function prefetchSocial(items){const refs=(items||[]).filter(x=>x.social_enabled&&x.donation_ref).slice(0,4);refs.forEach(x=>window.KiaDonationSocial?.seed?.(x));setTimeout(()=>refs.forEach((x,i)=>setTimeout(()=>window.KiaDonationSocial?.prefetch?.(x.donation_ref),i*240)),120)}
