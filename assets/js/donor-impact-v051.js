...
function renderAvatar(url,name){const root=$('[data-profile-avatar]');if(!root)return;const pref=readAvatarPrefV0519();
  ...
  root.innerHTML=url?`<img src="${esc(url)}" alt="Foto profil">`:`<span>${esc(initials(name))}</span>`;renderDefaultAvatarChoices(pref?.mode==='icon'?pref.icon:'')}
