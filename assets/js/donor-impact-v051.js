...
function restructure(){
  ...
  if(account&&!account.dataset.phaseC){account.dataset.phaseC='1';const card=account.querySelector('.section-card'),info=card?.querySelector('.system-info-list');if(card&&info){
    const wrap=document.createElement('div');wrap.className='account-overview-v0512';info.parentNode.insertBefore(wrap,info);const avatar=document.createElement('div');avatar.className='avatar-card-v0512';avatar.innerHTML=`<div class="avatar-v0512" data-profile-avatar><span>${esc(initials(KiaAuth.getUser()?.full_name))}</span></div><div><strong>Foto Profil</strong><p class="muted mini">JPG / PNG / WEBP · maks. 2 MB · rasio 1:1 disarankan.</p><label class="btn btn-soft">Ganti Foto<input type="file" accept="image/jpeg,image/png,image/webp" data-avatar-input hidden></label></div>`;wrap.appendChild(avatar);const right=document.createElement('div');right.appendChild(info);const actions=card.querySelector('.form-actions');if(actions)right.appendChild(actions);wrap.appendChild(right);
