(()=>{
  ...
  const key=p=>`kia_program_donations_v056_${programId}_${p}`;
  function readCache(p){try{for(const k of [key(p),`kia_program_donations_v055_${programId}_${p}`]){const x=JSON.parse(sessionStorage.getItem(k)||'null');if(x&&Date.now()-x.saved_at<CACHE_TTL)return x.data}return null}catch{return null}}
  ...
  function avatar(x){const donor=x.donor_label||'Hamba Allah',p=x.donor_profile||null,url=String(p?.avatar_url||'').trim();return `<div class="donor-avatar-frame-v056 frame-${esc(frameCode(p))}" aria-hidden="true">${url?`<img src="${esc(url)}" alt="">`:`<span>${esc(initials(donor))}</span>`}</div>`}
  function row(x,index){const donor=x.donor_label||'Hamba Allah',message=String(x.message||'').trim();return `<article class="donation-row-v056" data-donation-ref="${esc(x.donation_ref||'')}" tabindex="0" role="button" aria-label="Buka timeline donasi ${esc(donor)}">
    <div class="donation-cell-no-v056">#${index}</div>
    <div class="donation-cell-profile-v056">${avatar(x)}</div>
    ...
    <div class="donation-cell-program-v056">${esc(x.program_name||'Program KIA')}</div>
    <div class="donation-cell-badge-v056"><span class="donor-badge donor-badge--${badgeClass(x.donor_badge_code)}">${esc(x.donor_badge||'Umum')}</span></div>
    <div class="donation-cell-amount-v056"><strong>${idr(x.amount)}</strong><time>${esc(dateTime(x.paid_at))}</time></div>
  </article>`}
  function render(data,{refreshing=false}={}){
    const root=$('[data-donation-list]');page=Number(data.page)||1;totalPages=Number(data.total_pages)||1;total=Number(data.total)||0;const program=data.program||{};
