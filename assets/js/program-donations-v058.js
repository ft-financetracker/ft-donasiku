(()=>{
  ...
  const key=p=>`kia_program_donations_v058_${programId}_${p}`;
  ...
  function avatar(x){const donor=x.donor_label||'Hamba Allah',p=x.donor_profile||null,url=String(p?.avatar_url||'').trim();return `<div class="donor-avatar-frame-v058 frame-${esc(frameCode(p))}" aria-hidden="true">${url?`<img src="${esc(url)}" alt="">`:`<span>${esc(initials(donor))}</span>`}</div>`}
