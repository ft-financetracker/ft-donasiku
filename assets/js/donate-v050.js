(()=>{
  ...
  function paymentShellKey(token){return 'kia_payment_shell_v055_'+String(token||'').slice(-48)}
  ...
  async function loadProgram(){
    ...
    let cached=null;try{for(const key of ['kia_program_detail_v055_'+id,'kia_program_detail_v054_'+id,'kia_program_detail_v052_'+id,'kia_program_detail_v051_'+id]){const x=JSON.parse(localStorage.getItem(key)||'null');if(x&&Date.now()-x.saved_at<6*3600000){cached=x.data;break}}}catch{}
    ...
    try{
      ...
      program=r.data.program;try{localStorage.setItem('kia_program_detail_v055_'+id,JSON.stringify({saved_at:Date.now(),data:r.data}))}catch{}fillProgram();prefillUser();
  ...
  async function submit(e){
    ...
    try{
      ...
      location.replace('./payment.html?token='+encodeURIComponent(token)+'&v=055');
    }catch(err){
